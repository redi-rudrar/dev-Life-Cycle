...
int FeedTaskMgr::Startup(int maxUsers, int taskBlockSize)
{
...
TASKENTRY *FeedTaskMgr::AllocTask()
{
	if(!feedUsers)
		return 0;
	if(freedTasks)
...
int FeedTaskMgr::FreeTask(TASKENTRY *ptask)
{
...
FeedUser *FeedTaskMgr::AddUser(int PortType, int PortNo)
{
	if((PortType!=WS_USR)||(PortNo<0)||(PortNo>MAX_USERS))
		return 0;
	if(!feedUsers)
		return 0;
...
int FeedTaskMgr::RemUser(int PortType, int PortNo)
{
	if((PortType!=WS_USR)||(PortNo<0)||(PortNo>MAX_USERS))
		return -1;
	if(!feedUsers)
...
#ifdef FEEDTASK_STRINGKEY
TASKITEM *FeedTaskMgr::FindUserItem(const string& skey)
{
...
#else
TASKITEM *FeedTaskMgr::FindUserItem(void *uitem)
{
...
#endif
#ifdef FEEDTASK_STRINGKEY
TASKITEM *FeedTaskMgr::AddUserItem(const string& skey)
{
...
#else
TASKITEM *FeedTaskMgr::AddUserItem(void *uitem, HANDLE umutex)
{
...
#endif
#ifdef FEEDTASK_STRINGKEY
int FeedTaskMgr::DelUserItem(const string& skey)
{
	TASKITEMMAP::iterator iit=itemMap.find(skey);
	if(iit==itemMap.end())
		return -1;
...
#else
int FeedTaskMgr::DelUserItem(void *uitem)
{
	TASKITEMMAP::iterator iit=itemMap.find((DWORD)uitem);
	if(iit==itemMap.end())
		return -1;
...
#endif
#ifdef FEEDTASK_STRINGKEY
int FeedTaskMgr::CountTasks(const string& skey)
{
	TASKITEMMAP::iterator iit=itemMap.find(skey);
	if(iit==itemMap.end())
		return -1;
...
#else
int FeedTaskMgr::CountTasks(void *uitem)
{
	TASKITEMMAP::iterator iit=itemMap.find((DWORD)uitem);
	if(iit==itemMap.end())
		return -1;
...
#ifdef FEEDTASK_STRINGKEY
TASKENTRY *FeedTaskMgr::FindTask(FeedUser *fuser, const string& skey, uchar taskid)
{
...
TASKENTRY *FeedTaskMgr::FindUserTask(FeedUser *fuser, const string& skey, uchar taskid)
{	
...
#else
TASKENTRY *FeedTaskMgr::FindTask(FeedUser *fuser, void *uitem, uchar taskid)
{
	for(TASKENTRY *stask=fuser->ptasks;stask;stask=stask->unext)
	{
...
// This is effectively the same as FindTask, but is more efficient when the user has hundreds of tasks.
TASKENTRY *FeedTaskMgr::FindUserTask(FeedUser *fuser, void *uitem, uchar taskid)
{	
...
#ifdef FEEDTASK_STRINGKEY
TASKENTRY *FeedTaskMgr::AddTask(FeedUser *fuser, const string& skey, uchar taskid, void *udata, DWORD rid, TASKCALLBACK cbfunc)
{
	if(!fuser->authed)
		return 0;
	Lock();
...
#else
TASKENTRY *FeedTaskMgr::AddTask(FeedUser *fuser, void *uitem, HANDLE umutex, uchar taskid, void *udata, DWORD rid, TASKCALLBACK cbfunc)
{
	if(!fuser->authed)
		return 0;
	Lock();
...
#ifdef FEEDTASK_STRINGKEY
int FeedTaskMgr::DelTask(FeedUser *fuser, const string& skey, uchar taskid)
{
	Lock();
	TASKENTRY *ptask;
...
#else
int FeedTaskMgr::DelTask(FeedUser *fuser, void *uitem, uchar taskid)
{
	Lock();
	TASKENTRY *ptask;
	for(ptask=fuser->ptasks;ptask;ptask=ptask->unext)
...
#endif
int FeedTaskMgr::DelUserTask(TASKENTRY *dtask)
{
	Lock();
	//char dbuf[256]={0};
...
int FeedTaskMgr::DelUserTasks(FeedUser *fuser)
{
	Lock();
	while(fuser->ptasks)
...
#ifdef FEEDTASK_STRINGKEY
int FeedTaskMgr::CallTasks(const string& skey, uchar taskid, void *hint)
{
	TASKITEM *titem=FindUserItem(skey);
	if(!titem)
		return -1;
...
int FeedTaskMgr::CallItemTasks(TASKITEM *titem, uchar taskid, void *hint)
{
	if(!titem)
	{
		_ASSERT(false);
...
//int FeedTaskMgr::CallTasks(void *uitem, uchar taskid, void *hint)
//{
//	TASKITEM *titem=FindUserItem(skey);
//	if(!titem)
//		return -1;
...
#endif
int FeedTaskMgr::DelItemTask(TASKENTRY *dtask)
{
	Lock();
	//char dbuf[256]={0};
...
int FeedTaskMgr::DelItemTasks(TASKITEM *titem)
{
	Lock();
	while(titem->ptasks)
...
long FeedTaskMgr::NextSchedDate(bool days[7], bool allowToday)
{
	time_t tnow=time(0);
...
TASKENTRY *FeedTaskMgr::AddScheduledTask(FeedUser *fuser, uchar taskid, void *udata, TASKCALLBACK cbfunc,
...
int FeedTaskMgr::DelScheduledTask(FeedUser *fuser, uchar taskid)
{
	int ndel=0;
	Lock();
	for(SCHEDULE::iterator sit=schedule.begin();sit!=schedule.end();)
	{
		SCHEDENTRY *ptask=*sit;
		if((ptask->fuser==fuser)&&(ptask->taskid==taskid))
		{
			sit=schedule.erase(sit);
			delete ptask; ndel++;
		}
		else
			sit++;
...
int FeedTaskMgr::CheckSchedule(long wdate, long wtime)
{
	WSDate=wdate;
...
int FeedTaskMgr::RunScheduledTaskNow(SCHEDENTRY *pse, long wdate, long wtime)
{
	for(SCHEDULE::iterator sit=schedule.begin();sit!=schedule.end();sit++)
	{
		SCHEDENTRY *ptask=*sit;
