...
typedef struct tdTradeRec //Master Record for all Trades
{
	...
	char Gtc;				// 0=none, 1=gtd, 2=gtc, 3=gtx was T/F 0/1
	char Ext;					// True / False
...
//	char RouteFundId[21];		// SPARE Fund Id attached to the route agreement above
	...
	{
		char MarketMakerID[4];		// dt#4792 - kma - 20090129 = Market Maker Capacity - OSOR(S) - BigTradeRec
...
typedef struct tdOrderDetPkt_3
{
	...
	char OrderNo[ORDERNO_LEN];
	char LastTime[9];
	// begin DT6212 - kma - 20091005 - INTERNAL - Add new fields for FE to ORDERDETPKT_3
	union
	{
		char Aon;					// bit0=AON bit1=IOC/ArcaNow bit2=OnOpen bit3=OnClose bit4=IsldBookOnly bit5=FOK bit6=NyseDirect
		struct
	...
	// end DT6212 - kma - 20091005 - INTERNAL - Add new fields for FE to ORDERDETPKT_3
    ...
    union {
        ...
        struct {
            unsigned char  MasterSlave		: 2;
		        ...
		        bool	   ClosingTrade		: 1;
				bool	   RouteTagOrder	: 1;	// DT#5191 - Add RouteTagOrder field to frontend messages
				bool	   FacilitatedOrder	: 1;	// DT5453 Bud 05/14/09
				bool	   spare7			: 1;	//
				bool	   spare8			: 1;	//
				bool       spare9			: 1;
				bool	   spare10			: 1;
	...
	//char Spare[2]; Spare says coffer's tool
	long ClientRef;			// DT6047 - KMA - 9/9/9  INTERNAL ONLY - Add TradeDetRef to ORDERDETPKT_3 for FE updates (no qa)
	long TradeDetRef;
	char User[USER_LEN];
	char PrefMMId[ECN_LEN];
	...
	union {
        ...
        struct {
			...
			unsigned char	 MidPointMatch : 1;	//7 MPM-Lmt and MPM-Mkt order type DT6957 JAL 20100901 // DT6958 bud 08/25/10
            unsigned char    OT_8 : 1; //8 OrderType PEGS ONLY
		};
	};
	union {
...
//            unsigned char EASE_ExecMode_deleted		: 4;	//20EASE Execution Mode
	...
	long ReservedShares;
	float DiscretionaryAmount;	// Amount for Discretionary Order OR Used for Pegged if ISLD 1=PegMkt 2=PegBest
	char Liquidity2Bytes[2];	// DT6341 - 11/9/09 - kma - XTrade Gap (CL) - Pass Native Liquidity Value to FE

	char SpareSet[12];			// DT6341 - 11/9/09 - kma - XTrade Gap (CL) - Pass Native Liquidity Value to FE
